...
//        currentTurn === WHITE_TEAM ? alert(json.white[selectedPiece.position].piece)
//        : alert(json.black[selectedPiece.position].piece);
      //  alert(selectedPiece.position);
	if (enemyPiece !== null) {
...
function processMove(clickedBlock) 
{
	...
	if (pieceAtBlock !== null) {
		...
		checkIfPieceClicked(clickedBlock);
            ...
            alterContent(clickedBlock,selectedPiece,enemyPiece);
...
function alterContent(clickedBlock,selectedPiece)
{
       
           
          var  url = "ChessBoardServlet?Crow=" + clickedBlock.col+"&Ccol="+clickedBlock.row+"&Srow="+ selectedPiece.col+"&Scol="+selectedPiece.row;
          if (rcvReq.readyState == 4 || rcvReq.readyState == 0) {
                rcvReq.open("GET", url, true);
...
//	PIECE_QUEEN = 4,
    ...
    {       
        var str = rcvReq.responseText.toString().trim();
        if(str.length == 6)
        {
            movePiece(clickedBlock, enemyPiece);
            var text = (currentTurn === WHITE_TEAM ? "White Move" : "Black Move");
            document.getElementById('header').innerHTML = text;
        }
        else if(str.length == 4)
        {  
            var Piece = null;
            if(confirm(" Queen? ") == false)
            {
                if(confirm("BISHOP?") == false)
                {
                   if(confirm("ROUKE?")==false)
                   {
                       Piece =  PIECE_CASTLE;                       
                   }
                   else
            ...
            currentTurn === WHITE_TEAM ? (json.white[selectedPiece.position].piece = Piece)
            : (json.black[selectedPiece.position].piece = Piece);
            movePiece(clickedBlock, enemyPiece);
            var  url = "ChessBoardServlet?piece=" +Piece;
            balterContent(url);
       }
        else
        {
